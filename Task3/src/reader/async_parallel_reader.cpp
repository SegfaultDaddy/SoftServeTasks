...
void AsyncParallelReader::process_files(const std::filesystem::path& startDirectory, const std::vector<std::string_view>& extensions)
{
...
void AsyncParallelReader::read_directory_and_process_files(const std::filesystem::path& startDirectory, const std::vector<std::string_view>& extensions)
{
    ...
    for(const auto& entry : std::filesystem::directory_iterator{startDirectory})
    {
        ...
        {
            futures.push_back(std::async(std::launch::async, &AsyncParallelReader::read_directory_and_process_files, this, entry, extensions));
        }
    }
